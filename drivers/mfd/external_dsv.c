...
static char *mode_reg_name[] = {
	"normal_mode_reg",
	"lpwg_mode_reg",
	"lpwg_always_on_mode_reg",
	"lpwg_toggle_mode_reg",
	"sleep_mode_reg",
	"positive_only_on_mode_reg",
	"normal_without_delay_mode_reg",
	"power_off_mode_reg",
	"power_off_hiz_mode_reg",
	"output_voltage_mode_reg",
};

struct ext_dsv_platform_data {
	u32 *mode_reg[DSV_MODE_MAX_NUM];
	u32 mode_reg_len[DSV_MODE_MAX_NUM];
	int access_level;
	int ena_gpio;
};
...
int ext_dsv_mode_change_privileged(int mode)
{
	int ret = 0, i;
	...
	pr_info("ext_dsv_mode_changed: %s\n", mode_reg_name[mode]);
	if (pdata->mode_reg_len[mode] == 0) {
		pr_err("ext_dsv_mode_changed: mode %d reg is not defined\n", mode);
	}

	for (i = 0; i < pdata->mode_reg_len[mode]; i+=3) {
		ret += i2c_smbus_write_byte_data(cl, pdata->mode_reg[mode][i], pdata->mode_reg[mode][i+1]);
		usleep_range(pdata->mode_reg[mode][i+2] * 1000, pdata->mode_reg[mode][i+2] * 1000);
		pr_info("%s: DSV addr(0x%x),value(0x%x),delay(%dms)\n", __func__,
				pdata->mode_reg[mode][i], pdata->mode_reg[mode][i+1], pdata->mode_reg[mode][i+2]);
	}
	if (ret > 0)
...
int ext_dsv_mode_change_unprivileged(int mode)
{
	if (ext_dsv_base == NULL || ext_dsv_base->dev == NULL) {
		pr_err("%s: %s is not probed\n", __func__, DRIVER_NAME);
		return -ENODEV;
	...
	return ext_dsv_mode_change_privileged(mode);
}
...
static void parse_ext_dsv_dt(struct device_node *node, struct ext_dsv_platform_data *pdata)
{
	...
	for (i = 0; i < DSV_MODE_MAX_NUM; i++) {
		parse_ext_dsv_mode_reg_dt(dsv_node, mode_reg_name[i], &(pdata->mode_reg[i]), &(pdata->mode_reg_len[i]));
	}
