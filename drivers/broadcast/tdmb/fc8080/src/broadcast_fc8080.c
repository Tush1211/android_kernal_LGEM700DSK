...
#define FEATURE_DMB_USE_VDD_FOR_QCOM
//#define SPI_READ_WRITE_TEST
...
#ifdef FEATURE_DMB_USE_VDD_FOR_QCOM
struct vreg_config {
    char *name;
    unsigned long vmin;
    unsigned long vmax;
...
static const struct vreg_config const vreg_conf[] = {
    { "tdmb-ldo", 1800000UL, 1800000UL, 6000, },
    { "ant-sw-io", 2850000UL, 2850000UL, 6000, },
};
#endif
...
struct tdmb_fc8080_ctrl_blk
{
    ...
    struct pinctrl_state *pins_int_enable, *pins_int_disable;
...
#ifdef FEATURE_DMB_USE_VDD_FOR_QCOM
static int vreg_setup(const char *name, bool enable)
{
    size_t i;
    int rc;
    struct regulator *vreg;
    struct device *dev = &fc8080_ctrl_info.spi_ptr->dev;

    for (i = 0; i < ARRAY_SIZE(fc8080_ctrl_info.vreg); i++) {
...
int tdmb_fc8080_power_on_retry(void)
{
    ...
    for(i = 0 ; i < 10; i++)
    {
        ...
        if(!fc8080_ctrl_info.dmb_use_external_xtal) {
...
#ifdef SPI_READ_WRITE_TEST
    tdmb_fc8080_spi_write_read_test();
#endif
...
static int tdmb_pinctrl_init_fc8080(void)
{
    ...
    /* get pinctrl pointer */
    fc8080_ctrl_info.pinctrl_gpios = devm_pinctrl_get(&fc8080_ctrl_info.spi_ptr->dev);
    ...
    fc8080_ctrl_info.pins_int_enable = pinctrl_lookup_state(fc8080_ctrl_info.pinctrl_gpios, "irq_enable");
    if (IS_ERR(fc8080_ctrl_info.pins_int_enable)) {
        printk("%s can't find tdmb pinctrl irq_enable\n", __func__);
        return -EINVAL;
    }

    fc8080_ctrl_info.pins_int_disable = pinctrl_lookup_state(fc8080_ctrl_info.pinctrl_gpios, "irq_disable");
    if (IS_ERR(fc8080_ctrl_info.pins_int_disable)) {
...
static int tdmb_pinctrl_init(void)
{
    ...
    printk("[%s] start getting pinctrl\n", __func__);
...
void tunerbb_drv_hw_setting(void)
{
    ...
    // IRQ
    pinctrl_select_state(fc8080_ctrl_info.pinctrl_gpios, fc8080_ctrl_info.pins_int_disable);
...
void tunerbb_drv_hw_init(void)
{
...
#ifdef FEATURE_DMB_USE_VDD_FOR_QCOM
    vreg_setup("tdmb-ldo", true);
    vreg_setup("ant-sw-io", true);
#endif
    ...
    // IRQ
    pinctrl_select_state(fc8080_ctrl_info.pinctrl_gpios, fc8080_ctrl_info.pins_int_enable);

    // LNA
    if(!IS_ERR_OR_NULL(fc8080_ctrl_info.pins_lna_low)) {
        printk("[%s] lna pin setting\n",__func__);
        pinctrl_select_state(fc8080_ctrl_info.pinctrl_gpios, fc8080_ctrl_info.pins_lna_low);
    }

    // ANT Switch
    if(!IS_ERR_OR_NULL(fc8080_ctrl_info.pins_ant_sw_enable)) {
        printk("[%s] antenna switch pin setting\n",__func__);
        pinctrl_select_state(fc8080_ctrl_info.pinctrl_gpios, fc8080_ctrl_info.pins_ant_sw_enable);
    }
...
#ifdef FEATURE_DMB_USE_VDD_FOR_QCOM
    vreg_setup("tdmb-ldo", false);
    vreg_setup("ant-sw-io", false);
#endif
...
static int tunerbb_drv_load_hw_configure(struct spi_device *spi)
{
    ...
    fc8080_ctrl_info.dmb_main_node = fc8080_ctrl_info.pdev->dev.of_node;
    if(fc8080_ctrl_info.dmb_main_node)
    {
        of_property_read_u32(fc8080_ctrl_info.dmb_main_node, "use-external-xtal", &fc8080_ctrl_info.dmb_use_external_xtal);
        of_property_read_u32(fc8080_ctrl_info.dmb_main_node, "xtal-freq", &fc8080_ctrl_info.dmb_xtal_freq);
        ...
        printk("%s: use_xtal : %d\n", __func__, fc8080_ctrl_info.dmb_use_external_xtal);
        printk("%s: dmb_xtal_freq : %d\n", __func__, fc8080_ctrl_info.dmb_xtal_freq);
    }

    fc8080_ctrl_info.dmb_lna_node = of_find_compatible_node(NULL, NULL, "lge,tdmb_lna");
    if(fc8080_ctrl_info.dmb_lna_node)
    {
    ...
    fc8080_ctrl_info.dmb_ant_switch_node = of_find_compatible_node(NULL, NULL, "lge,tdmb_ant_switch");
    if(fc8080_ctrl_info.dmb_ant_switch_node)
    {
